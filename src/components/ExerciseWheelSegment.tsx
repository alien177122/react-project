  ...
  return (
    <g>
      <path
        ...
        stroke={isSelected || isHovered ? 'var(--ta-calc-accent, #ff6b35)' : 'transparent'}
      ...
      {saved ? <SavedOneRmLabel saved={saved} color={color} flip={flip} midDeg={midDeg} cx={cx} cy={cy} ro={ro} /> : null}
    </g>
  )
}

function SavedOneRmLabel({ saved, color, flip, midDeg, cx, cy, ro }: {
  saved: SavedExercise
  color: string
  flip: boolean
  midDeg: number
