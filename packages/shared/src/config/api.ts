...
export const API_ENV_CONFIG: Record<ApiEnv, Pick<ApiConfig, 'baseUrl' | 'timeoutMs' | 'retryCount'>> = {
  dev: {
...
const ENV_KEYS = {
  ...
  allowOfflineMode: ['ALLOW_OFFLINE_MODE', 'VITE_ALLOW_OFFLINE_MODE', 'EXPO_PUBLIC_ALLOW_OFFLINE_MODE'],
} as const
...
function parseApiEnv(value: unknown): ApiEnv | undefined {
  ...
  return (API_ENVS as readonly string[]).includes(normalized)
    ? (normalized as ApiEnv)
    : undefined
}
...
export function buildApiConfig({
  ...
  const timeoutMs = parseTimeoutMs(envTimeout)
  ...
  const retryCount = parseRetryCount(envRetryCount)
