...
function displayWeight(config: ExerciseConfig, result: SavedExercise, rawWeight: number): number {
  if ((config.usesBodyWeight || config.isPullup) && result.bodyWeight != null) {
    return rawWeight - result.bodyWeight;
  }
  return rawWeight;
}
...
export function usePeriodization(config: ExerciseConfig, result: SavedExercise) {
  return useMemo(() => {
    ...
    return {
      ...
      // 7 horizontal lines — same density as JournalProgressChart / `.ta-period` etalon
