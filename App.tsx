...
function VolumeDonut() {
  ...
  // ⚡ Bolt: computeMuscleVol calculates total volumes across all exercises
  // We can memoize it (although it currently only depends on static EXERCISES)
  // to ensure its referential identity is stable, then use it as a dependency
  const vol = useMemo(() => computeMuscleVol(), [])

  // ⚡ Bolt: Memoize expensive O(N^2) segment and arc computations
  // Recalculates when volume changes, saving CPU cycles on hover state changes
  const { segs, catArcs, catVols, total } = useMemo(() => {
    ...
    return { segs, catArcs, catVols, total }
  }, [vol])
...
function App() {
  ...
  // ⚡ Bolt: Replace O(N*M) lookups (.some inside .every/.filter) with O(1) Set lookups
  // Mappings are calculated once per render instead of N times
  const { allSaved, missingExercises } = useMemo(() => {
    if (!userData) return { allSaved: false, missingExercises: Object.values(EXERCISES).map(ex => ex.name) }

    const savedKeys = new Set(userData.exercises.map(e => e.exerciseKey))
    const missing = Object.entries(EXERCISES)
      .filter(([k]) => !savedKeys.has(k))
      .map(([, ex]) => ex.name)

    return { allSaved: missing.length === 0, missingExercises: missing }
  }, [userData])
