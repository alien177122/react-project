...
function VolumeDonut() {
  ...
  // ⚡ Bolt: Isolate pure static computation to safely memoize volume data
  const vol = useMemo(() => computeMuscleVol(), [])

  // ⚡ Bolt: Consolidate multiple derived values into a single cached computation.
  // This prevents redundant calculations of total, segs, catArcs, and catVols
  // on every re-render (which is critical during high-frequency hover events).
  const { total, segs, catArcs, catVols } = useMemo(() => {
    ...
    const segs: DonutSeg[] = []
    const catArcs: Array<{ catKey: string; startDeg: number; endDeg: number; color: string }> = []
