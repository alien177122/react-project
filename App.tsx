...
function App() {
  ...
  // ⚡ Bolt: Reduced O(N*M) existence checks to O(1) using a Set cache inside useMemo.
  // This avoids redundant iterations over userData.exercises on every App render.
  const { allSaved, missingExercises } = useMemo(() => {
    if (!userData) {
      return {
        allSaved: false,
        missingExercises: Object.entries(EXERCISES).map(([, ex]) => ex.name)
      }
    }
    const savedKeys = new Set(userData.exercises.map(e => e.exerciseKey))
    return {
      allSaved: Object.keys(EXERCISES).every(k => savedKeys.has(k)),
