...
function App() {
  ...
  // ⚡ Bolt: Cache saved exercise keys in a Set for O(1) lookup instead of O(N*M) with .some() inside .filter()/.every()
  const savedExerciseKeys = useMemo(
    () => new Set(userData ? userData.exercises.map(e => e.exerciseKey) : []),
    [userData]
  )
