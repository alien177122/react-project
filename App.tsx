...
function TrainingTab({ userData, token, setUserData, allSaved, missingExercises, setActiveTab }: TrainingTabProps) {
  ...
  const savedExercisesMap = useMemo(() => {
    // ⚡ Bolt: Cache saved exercises in a Map for O(1) lookup instead of O(N) array search on every render
...
function App() {
  ...
  const { allSaved, missingExercises } = useMemo(() => {
    if (!userData) return { allSaved: false, missingExercises: [] }

    // ⚡ Bolt: Cache saved exercise keys in a Set for O(1) lookups instead of O(N*M)
