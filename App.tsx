...
function VolumeDonut() {
  ...
  const { vol, total, segs, catArcs, catVols } = useMemo(() => {
    ...
    for (const catKey of CAT_ORDER) {
      ...
      catArcs.push(muscles.length > 0 ? { catKey, startDeg, endDeg: deg, color: CAT_META[catKey].color } : null)
      deg += CAT_GAP
    }

    return { vol, total, segs, catArcs, catVols }
  }, []) // Empty dependency array because computeMuscleVol uses static constants
