...
function ProgressionBlock({ config, result }: { config: ExerciseConfig; result: SavedExercise }) {
  ...
  // ⚡ Bolt: memoized weekRows array mapping to prevent recalculation of working weights
  // on every render triggered by the hovRow (hover state) updates
  const weekRows = useMemo(() => {
    return config.percentages.map((pct, i) => {
