...
function VolumeDonut() {
  ...
  // ⚡ Bolt: Cache static volume computations and avoid redundant O(N) array filtering.
  // computeMuscleVol relies only on module-level constants (EXERCISES, MUSCLE_CONTRIB).
  ...
  const { total, segs, catArcs, catVols } = useMemo(() => {
    ...
    for (const catKey of CAT_ORDER) {
      ...
      if (muscles.length > 0) {
        catArcs.push({ catKey, startDeg: catStartDeg, endDeg: deg, color: CAT_META[catKey].color })
      } else {
        catArcs.push(null)
      }
      deg += CAT_GAP
