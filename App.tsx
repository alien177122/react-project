...
function VolumeDonut() {
  ...
  const volStr = JSON.stringify(vol)

  // ⚡ Bolt: wrap static derivations in useMemo to prevent recalculating on every hover interaction
  const { total, segs, catArcs, catVols } = useMemo(() => {
    const t = MUSCLE_ORDER.reduce((s, m) => s + (vol[m] || 0), 0)
    ...
    for (const catKey of CAT_ORDER) {
      ...
      const catDegTotal = (catVol / t) * usable
      ...
      for (let i = 0; i < muscles.length; i++) {
        ...
        sgs.push({ muscle: m, value: mVol, pct: (mVol / t) * 100, startDeg: deg, endDeg: deg + mDeg, catKey })
        deg += mDeg + (i < muscles.length - 1 ? SEG_GAP : 0)
      }
      deg += CAT_GAP
    }

    const cArcs = CAT_ORDER.map(catKey => {
      const cs = sgs.filter(s => s.catKey === catKey)
      if (!cs.length) return null
      return { catKey, startDeg: cs[0].startDeg, endDeg: cs[cs.length - 1].endDeg, color: CAT_META[catKey].color }
    })

    const cVols = CAT_ORDER.map(c => MUSCLE_ORDER.filter(m => MUSCLE_META[m].catKey === c).reduce((sum, m) => sum + (vol[m] || 0), 0))

    return { total: t, segs: sgs, catArcs: cArcs, catVols: cVols }
