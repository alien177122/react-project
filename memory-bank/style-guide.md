...
Эталон sec **03 Сохранённые (N/12)**: `memory-bank/reference/calc-saved-exercises-standard.md` — SectionBlock apple → `.ta-calc-saved-list` (gap 10px) → card grid `1fr auto`, padding 14×16, radius 12px; `__name` 600 / `__meta` 12px muted; `__one-rm` 16px `--ta-calc-accent`; `.is-active` = focus tint + 2px left rail; delete scoped 44px; empty = section hidden. **MUST** для journal/training rows в `.ta-shell` (см. §13 эталона).
