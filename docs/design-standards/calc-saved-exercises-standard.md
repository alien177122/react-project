...
- `platforms/legacy-mobile/app/(tabs)/calculator.tsx` — Pressable + StyleSheet; 1RM **без** orange accent; title `Сохранённые · N/M`
- `platforms/macos/src/screens/CalculatorScreen.tsx` — legacy SectionBlock без `variant="apple"`; title с `·`
- `platforms/mobile/app/(tabs)/index.tsx` — FlatList saved exercises, другой layout
