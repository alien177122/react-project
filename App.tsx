...
function ExerciseWheel({ value, onChange, savedExercises = [] }: { value: string; onChange: (key: string) => void; savedExercises?: SavedExercise[] }) {
  ...
  // ⚡ Bolt: Pre-process savedExercises into a Map for O(1) lookups during SVG rendering
  const savedMap = useMemo(() => {
    const map = new Map<string, SavedExercise>()
    for (const ex of savedExercises) map.set(ex.exerciseKey, ex)
    return map
  }, [savedExercises])
...
function TrainingTab({ userData, token, setUserData, allSaved, missingExercises, setActiveTab }: TrainingTabProps) {
  ...
  // ⚡ Bolt: Cache userData.exercises into a Map for O(1) lookups during daily schedule computation
...
function App() {
  ...
  // ⚡ Bolt: Use Set to cache saved exercises for O(1) lookup instead of O(N*M) with .some()
