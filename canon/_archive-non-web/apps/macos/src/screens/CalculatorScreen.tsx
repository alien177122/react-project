...
export function CalculatorScreen(): React.JSX.Element {
  const {token, userName, userData, setUserData, handleLogout} = useAuthContext();
  const {
    selectedExercise,
    selectExercise,
    testWeight,
  ...
  const config =
    EXERCISES[activeResult?.exerciseKey || selectedExercise] ?? EXERCISES[selectedExercise];
    ...
    <ScreenLayout
      ...
      {activeResult ? (
        <SectionBlock num="02" title={`Прогрессия — ${config.name}`}>
          <View style={styles.insightCard}>
            ...
            {firstWeek && lastWeek ? (
              <Text style={styles.insightText}>
                {'\n'}Нед 1: {firstWeek.weight} кг · {firstWeek.scheme.sets}×{firstWeek.scheme.reps}{' '}
                = {firstWeek.totalReps} повт{'\n'}Нед 8: {lastWeek.weight} кг ·{' '}
                {lastWeek.scheme.sets}×{lastWeek.scheme.reps} = {lastWeek.totalReps} повт
              </Text>
            ) : null}
          </View>
          ...
          <View style={styles.noteBox}>
            <Text style={styles.noteText}>
              Неделя 5 — волновой откат: вес слегка снижается, объём восстанавливается. Нажимай на
              строки прогрессии, чтобы подсветить соответствующую неделю в графике.
            </Text>
          </View>
        </SectionBlock>
      ) : null}
      ...
      <SectionBlock num="04" title="Распределение объёма">
        <View style={styles.noteBox}>
          <Text style={styles.noteText}>
            Средние рабочие подходы за цикл, распределённые по мышечным группам. Нажимай на сектор
            или строку легенды, чтобы увидеть детали.
          </Text>
        </View>
        <VolumeDonut />
      </SectionBlock>
