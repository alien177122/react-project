...
function getSegmentOpacity(selectedId: string | null, segmentId: string, segmentCat: VolumeCategory): number {
  ...
  return (
    <aside className="ta-vol__detail is-active" aria-label={`Источники объёма: ${muscle.label}`}>
      <div className="ta-vol__detail-head">
        ...
        <h3>{muscle.label}</h3>
        <p>{formatNumber(muscle.value)} сет / цикл · {formatPercent(muscle.percent)} общего объёма</p>
      </div>
...
export default function VolumeDonut() {
  ...
  const selectedMuscle = useMemo(
    () => breakdown.muscles.find(muscle => muscle.id === selectedMuscleId) ?? breakdown.muscles[0] ?? null,
  ...
  const { segments, categoryArcs } = useMemo(() => {
    ...
    CAT_ORDER.forEach((catKey) => {
      const categoryMuscles = MUSCLE_ORDER
        .map(muscleId => muscleById.get(muscleId))
        .filter((muscle): muscle is MuscleBreakdown => Boolean(muscle && muscle.catKey === catKey))

      const catTotal = breakdown.categories[catKey].total
      const catDegTotal = breakdown.totalSets > 0 ? (catTotal / breakdown.totalSets) * usableDeg : 0
      const muscleUsable = Math.max(0, catDegTotal - SEGMENT_GAP * Math.max(0, categoryMuscles.length - 1))
      const catStart = deg
  ...
  return (
    <div className="ta-vol">
      <p className="ta-vol__lede">
        Показывает, какие мышечные группы получают больше всего рабочих подходов в базовом 3-дневном цикле.
      </p>
      ...
      <div className="ta-vol__grid">
        ...
        <div className="ta-vol__donut">
          ...
          >
            ...
            <desc id={descId}>
              Внешнее кольцо показывает жим, тягу и ноги. Внутреннее кольцо выбирает отдельную мышцу.
            </desc>
            ...
            {categoryArcs.map(category => (
              <path
                ...
                d={donutArc(DONUT_CENTER, DONUT_CENTER, OUTER_RADIUS_OUT, OUTER_RADIUS_IN, category.startDeg, category.endDeg)}
            ...
            {segments.map(segment => {
              ...
              return (
                <path
                  ...
                  d={donutArc(DONUT_CENTER, DONUT_CENTER, INNER_RADIUS_OUT, INNER_RADIUS_IN, segment.startDeg, segment.endDeg)}
                  fill={muscle.color}
            ...
            <g aria-hidden="true">
              <text x={DONUT_CENTER} y={DONUT_CENTER - 10} textAnchor="middle" className="ta-vol__center-label">
                {selectedMuscle?.label ?? 'Объём'}
              </text>
              <text x={DONUT_CENTER} y={DONUT_CENTER + 8} textAnchor="middle" className="ta-vol__center-value">
                {selectedMuscle ? formatNumber(selectedMuscle.value) : formatNumber(breakdown.totalSets)}
              </text>
              <text x={DONUT_CENTER} y={DONUT_CENTER + 24} textAnchor="middle" className="ta-vol__center-meta">
