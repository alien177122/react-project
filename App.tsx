...
function App() {
  ...
  // ⚡ Bolt: Optimize O(N*M) existence check to O(1) Set lookup
  const savedExerciseKeys = useMemo(() => {
    if (!userData?.exercises) return new Set<string>()
    return new Set(userData.exercises.map(e => e.exerciseKey))
  }, [userData])

  const allSaved = useMemo(() => {
    if (!userData) return false
    return Object.keys(EXERCISES).every(k => savedExerciseKeys.has(k))
