...
function VolumeDonut() {
  ...
  // ⚡ Bolt: computeMuscleVol relies only on static constants, safely memoized with []
  const vol = useMemo(() => computeMuscleVol(), [])

  // ⚡ Bolt: Consolidate multiple category-related iterations and static segment derivations to avoid O(N) work on hover
  const { total, segs, catArcs, catVols } = useMemo(() => {
    ...
    const segs: DonutSeg[] = []
    const catArcs: { catKey: string, startDeg: number, endDeg: number, color: string }[] = []
